...
namespace SimpleWeb
{
    ...
    public:
        DEPRECATED Server(unsigned short port, size_t thread_pool_size, const std::string &cert_file, const std::string &private_key_file,
                          ...
                          const std::string &verify_file = std::string()) : Server(cert_file, private_key_file, verify_file)
        {
            config.port = port;
        ...
        Server(const std::string &cert_file, const std::string &private_key_file, const std::string &verify_file = std::string()) : ServerBase<HTTPS>::ServerBase(443), context(boost::asio::ssl::context::tlsv12)
        {
        ...
        void start()
        {
            if (set_session_id_context)
            {
                ...
                // session_id_context = std::to_string(8000) + ':';
    ...
    protected:
        ...
        void accept()
        {
            ...
            // std::cout << "Server ip is " << socket->lowest_layer().local_endpoint() << std::endl;
